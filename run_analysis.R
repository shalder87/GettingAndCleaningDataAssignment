# scrit for the Assignment
# change to source the file location
...
codebook("# Code Book for the Assignment")
...
codebook("## Actions performed on the data:")
...
resultingData <- reshapedData
rm(reshapedData)
codebook("* renamed 'reshapedData' to **'resultingData'**")
log("variable 'resultingData' available for use: ", nrow(resultingData)," x ", ncol(resultingData))

# 5. Creates a second, independent tidy data set with the average of each variable for each activity and each subject. 
log("[#5] Appropriately labels the data set with descriptive variable names. ")
tidyData <- dcast(resultingData, activity_name + subject ~ variable, mean)
log("variable 'tidyData' available for use ", nrow(tidyData)," x ",ncol(tidyData))
codebook("* cast 'resultingData' into **'tidyData'** with the average of each variable for each activity and each subject dimensions:", nrow(tidyData)," x ", ncol(tidyData))
...
codebook("") 
codebook("## 'resultingData' variable\n")
